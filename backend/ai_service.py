...
def generate_topic_summary(topic_name, notes_context):
    prompt = f"""
Generate a structured study card for the topic: '{topic_name}'.
Grounded in the student's lecture notes provided below:

Notes Context:
\"\"\"{notes_context[:4000]}\"\"\"

Structure your response cleanly in Markdown with these exact sections:
### {topic_name}
**Definition**: A concise 1-2 sentence definition.
**Key Rules & Mechanics**: 3-4 bullet points detailing rules, syntax, or keywords.
**Example**: A clear, short code or conceptual example.
**Common Mistakes**: 2 common pitfalls or misconceptions students have about this concept.
"""
...
**Definition:**
{topic_name} is a fundamental concept covered in your notes that defines structural behavior and operational rules in this subject.

**Key Rules & Mechanics:**
- Adheres strictly to the syntax and design rules defined in your lecture notes.
- Must be declared with appropriate modifiers and initialized correctly.
- Directly depends on prerequisite foundational concepts.
- Governed by compile-time and runtime validation.

**Example:**
```
// Illustration of {topic_name}
// Refer to your course notes for specific implementations
```

**Common Mistakes:**
- Confusing compile-time checks with runtime behavior.
- Omitting required keywords or failing to meet access restrictions.
"""

# ==========================================
# 3. QUIZ GENERATION
# ==========================================
def generate_quiz_questions(topic_name, notes_context, num_questions=3):
    prompt = f"""
Generate {num_questions} multiple-choice questions for the topic '{topic_name}' based on the notes below.
...
[
  {{
    ...
    "correct_answer": "The exact matching text of the correct option",
...
"""
    ...
    return [
        {
            "question": f"Which statement best characterizes {topic_name} according to your lecture notes?",
            "options": [
                f"It represents a primary mechanism for structuring behavior in {topic_name}.",
                "It can be executed without adhering to prerequisite dependencies.",
                "It is deprecated in standard specifications.",
                "It has no impact on runtime execution."
            ],
            "correct_answer": f"It represents a primary mechanism for structuring behavior in {topic_name}.",
            "explanation": f"{topic_name} forms an essential structural element in your course curriculum."
        }
    ]
